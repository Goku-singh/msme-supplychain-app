...
st.set_page_config(page_title="MSME Supply Chain Dashboard", layout="wide")

st.title("📊 MSME Supply Chain Dashboard")
st.markdown("Upload your `.csv` or `.xlsx` file to get started.")
...
if uploaded_file:
    try:
        ...
        # Attempt to find a 'Date' column and parse it
        if "Date" in df.columns:
            df["Date"] = df["Date"].astype(str).str.strip()  # Clean whitespace
            df["Date"] = pd.to_datetime(df["Date"], dayfirst=True, errors='coerce')

            # Warn if any dates failed to parse
            if df["Date"].isna().sum() > 0:
                st.warning("Some date values could not be parsed and have been set as NaT (Not a Time).")
                st.dataframe(df[df["Date"].isna()])

        st.success("File uploaded and processed successfully.")
        st.dataframe(df)

        # Sample Plot if 'Date' and 'Quantity' columns exist
        if "Date" in df.columns and "Quantity" in df.columns:
            st.subheader("📈 Quantity Over Time")
            fig = px.line(df.dropna(subset=["Date"]), x="Date", y="Quantity", title="Quantity Trend")
            st.plotly_chart(fig, use_container_width=True)
        else:
            st.info("To view a line chart, make sure your data has 'Date' and 'Quantity' columns.")
